...
/**
 * <code>PSQLSettingsDAO</code>:n avulla voidaan hakea ja tallentaa
 * asetukset.
 *
 * @author Tommi Helineva
 */
public class PSQLSettingsDAO extends SQLSettingsDAO {
	private PSQLSession sess;

	/**
	 * Luo <code>PSQLSettingsDAO</code>-olion, joka käyttää
	 * tietokantaistuntoa <code>sess</code>
	 *
	 * @param sess tietokantaistunto
	 */
	public PSQLSettingsDAO(PSQLSession sess) {
		this.sess = sess;
	}
	...
	protected PreparedStatement getInsertQuery() throws SQLException {
		return sess.prepareStatement("INSERT INTO settings (version, name, business_id, current_period_id, document_type_id, properties) VALUES (15, ?, ?, ?, ?, ?)");
	}
